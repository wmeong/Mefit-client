    ...
    <v-row>
      ...
      <v-col cols="12" md="6" class="d-flex flex-column">
        ...
        <v-card class="modern-card" elevation="6">
          ...
          <div class="personal-color-result animated fadeIn" @click="navigateToPersonalColorPage">
          ...
          <v-row class="main-color">
            ...
            <v-col cols="9" class="d-flex">
              <v-avatar v-for="(color, index) in characterInfo.main_colors || ['#ccc']" :key="'main-color-' + index" :color="color" size="33" class="mr-2"></v-avatar>
            </v-col>
          </v-row>
          <v-row class="sub-color">
            ...
            <v-col cols="9" class="d-flex">
              <v-avatar v-for="(color, index) in characterInfo.sub_colors || ['#eee']" :key="'sub-color-' + index" :color="color" size="33" class="mr-2"></v-avatar>
            </v-col>
          </v-row>
        </v-card>
      </v-col>
      ...
      <v-col cols="12" md="6" class="d-flex flex-column">
        ...
        <v-card class="picker-card mb-4 animated fadeIn" elevation="12">
          <v-color-picker v-model="selectedColor" flat></v-color-picker>
        </v-card>

        <!-- 4번: 컬러 저장 영역 -->
        <v-card class="saved-colors-box animated popIn" elevation="6" :style="{ height: '360px' }">
          <v-btn block color="pink lighten-2" class="add-color-btn mt-5" @click="addColor" :disabled="!selectedColor">
            💖 색상 추가 💖
          </v-btn>
          <v-row dense justify="center" class="mt-4" style="max-height: 240px; overflow-y: auto;">
            <v-col v-for="(color, index) in savedColors" :key="index" cols="6" md="3" class="pa-2">
              <v-card :style="{ backgroundColor: color }" class="color-box powder-box" outlined>
...
export default {
  ...
  methods: {
    addColor() {
      if (this.savedColors.length < 24 && this.selectedColor && !this.savedColors.includes(this.selectedColor)) {
...
.color-picks-title {
  ...
  color: #ffffff;
  letter-spacing: 5px;
  text-shadow: 0 0 20px rgba(193, 101, 255, 0.8), 0 0 40px rgba(255, 223, 0, 0.6), 0 0 60px rgba(255, 223, 0, 0.4);
}
