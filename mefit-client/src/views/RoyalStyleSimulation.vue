            ...
            />
              <!-- 전광판 -->
                <div class="label-counter">
                    <p>쿠폰 사용 횟수: <span>{{ couponCount }}</span> 개</p>
                    <p>스페셜 라벨: <span>{{ specialLabelCount }}</span> 개</p>
                    <p>사용 캐시: <span>{{ usedCash }}</span> 캐시</p>
                </div>
        </div>
        <!-- 보물상자 애니메이션 -->
...
export default {
 ...
 formattedItemNames() {
            if (this.simulationResult.processedItemNames && this.simulationResult.processedItemNames.includes(",")) {
    ...
    window.addEventListener("keydown", this.skipAnimation);
        ...
        async showPopupResult() {
            try {
                ...
                // 스페셜 라벨 체크
                this.isSpecialLabel = this.simulationResult.originalItemName.includes("[스페셜 라벨]");
                ...
                if (this.recentResults.length > 40) {
...
skipAnimation(event) {
  ...
  this.confettiInterval = setInterval(() => {
        ...
        getPopupImages() {
            try {
                const imageNames = this.simulationResult.processedItemNames.split(",").map(name => name.trim());
                return imageNames.map(imageName => require(`@/assets/royalstyle/${imageName}.png`));
            } catch (error) {
